...
echo "$($PSQL "SELECT MAX(g.winner_goals) AS most_goals FROM games g INNER JOIN teams t ON t.team_id = g.winner_id GROUP BY team_id ORDER BY most_goals DESC LIMIT 1")"
